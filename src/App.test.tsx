...
describe('Interview management system', () => {
  ...
  it('shows a separate Qobooli-only matched report and exports it', async () => {
    ...
    apiApplicants = apiApplicants.map((applicant) => applicant.nationalId === '1122595406'
      ...
      : applicant.nationalId === '1125356871'
        ? { ...applicant, waitingNo: 'W-101', status: 'بانتظار المقابلة' }
      : applicant)
    render(<App />)
    ...
    expect(screen.getAllByText('بانتظار اكتمال الحضور والتقييم').length).toBeGreaterThan(0)
    ...
    expect(csv).toContain('حاضر')
    expect(csv).toContain('غير حاضر')
    expect(csv).toContain('بانتظار اكتمال الحضور والتقييم')
    expect(csv).toContain('45')
    expect(csv).toContain('ملاحظة تقييم قبولي')
    expect(csv).toContain('منصة قبولي')
    expect(csv).not.toContain('اسم الكلية')
    expect(csv).not.toContain('القسم')
    expect(csv).not.toContain('رئيس القسم / رئيس اللجنة')
    expect(csv).not.toContain('مسؤول إدارة الكلية')
    expect(csv).not.toContain('وكيل شؤون المتدربين')
    expect(csv).not.toContain('المنشأة')
    expect(csv).not.toContain('البرنامج')
  ...
  it('opens a printable Qobooli PDF report', async () => {
    ...
    expect(html).not.toContain('اسم الكلية')
    expect(html).not.toContain('القسم')
    expect(html).not.toContain('رئيس القسم / رئيس اللجنة')
    expect(html).not.toContain('مسؤول إدارة الكلية')
    expect(html).not.toContain('وكيل شؤون المتدربين')
    expect(html).not.toContain('<th>المنشأة</th>')
    expect(html).not.toContain('<th>البرنامج</th>')
  ...
  it('exports applicants to a CSV file without college identity or request/program fields', async () => {
    const user = userEvent.setup()
    ...
    expect(csv).not.toContain('مسؤول إدارة الكلية')
    expect(csv).not.toContain('وكيل شؤون المتدربين')
    expect(csv).not.toContain('رقم الطلب')
    expect(csv).not.toContain('البرنامج')
    expect(csv).not.toContain('REQ-2026-0001')
    expect(csv).not.toContain('30487')
  ...
  it('opens a printable PDF report without college identity or request fields', async () => {
    const user = userEvent.setup()
    const write = vi.fn()
    const close = vi.fn()
    vi.spyOn(window, 'open').mockReturnValue({
    ...
    expect(html).not.toContain('مسؤول إدارة الكلية')
    expect(html).not.toContain('وكيل شؤون المتدربين')
    expect(html).not.toContain('<th>رقم الطلب</th>')
    expect(html).not.toContain('REQ-2026-0001')
  ...
  it('does not show public visual analytics in the applicant-only page', () => {
    ...
    expect(screen.queryByRole('button', { name: 'تصورات متقدمة' })).toBeNull()
